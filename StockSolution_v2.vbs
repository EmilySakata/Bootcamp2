...
' Total change in the stock in column B
       	avg_day = avg_day + (Cells(i, 6).Value - Cells(i, 3).Value)
       	sheet2.Cells(j + 2, 2).Value = Round(avg_day, 2)
' The Percent of change in column C
 		per_change = Round((avg_day / Cells(start, 3)) * 100, 2)
...
' The total change in stock should be red if there is a loss and green for a gain.
        ...
        ' Select Case sheet2.Cells(i, 2).Value
        ' 	Case Is > 0
        ...
        ' End Select
 		...
 		Select Case change
            Case Is > 0
               sheet2.Range("B" & 2 + j).Interior.ColorIndex = 4
            Case Is < 0
                sheet2.Range("B" & 2 + j).Interior.ColorIndex = 3
            Case Else
                sheet2.Range("B" & 2 + j).Interior.ColorIndex = 0
        End Select
